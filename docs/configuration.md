...
file-based overrides such as a different database, a custom `ILLO_PRIVATE_HOME`,
or development-only provider-key fallbacks.
...
file and should not be committed, but it is not the place for model provider
API keys.
