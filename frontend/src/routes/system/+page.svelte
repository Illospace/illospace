<script lang="ts">
  ...
  async function connectWithApiKey() {
    ...
    try {
      ...
      apiKey = '';
      notice = { tone: 'success', title: 'OpenAI API key connected.' };
  ...
  async function connectWithOpenAIEmbeddingKey() {
    ...
    try {
      ...
      notice = {
        tone: 'success',
        title: 'Memory key saved.',
        detail: 'Memory and retrieval can use OpenAI embeddings.',
      };
    } catch (error) {
  ...
  function connectionSummary() {
    if (!settings || settings.connection.status !== 'connected') {
      return 'Connect Codex or OpenAI for models. API keys can also power installation memory.';
    }
    const label = settings.connection.label || (settings.connection.method === 'chatgpt' ? 'Codex / ChatGPT' : 'OpenAI API key');
    return `${label} for models. Memory keys apply to this installation.`;
  }
  ...
  function memorySummary() {
    ...
    return `${optionLabel(settings.memory.embedder_options, memoryDraft.embedder)} - ${dimensions || 'unknown'} dimensions, installation-wide`;
  }
    ...
    return [
      {
        ...
        detail: accessDone
          ? connectionSummary()
          : 'Sign in with Codex or paste an OpenAI key. Add an API key when cloud memory uses OpenAI or Gemini.',
      },
      {
        key: 'models',
        title: 'Choose Models',
  ...
  function memoryGuideDetail(memoryDone: boolean, memoryBlocked: boolean) {
    if (!settings) return 'Choose one installation-wide embedder, then Save & check.';
    if (!canManageSettings) return 'An owner or admin chooses one installation-wide embedder and runs Save & check.';
    if (memoryBlocked) return 'Existing installation vectors need a rebuild before changing embedder or embedding model.';
    if (memoryDone) return `${localEmbedderLabel(settings.memory.embedder)} memory is ready across this installation.`;
    if (usesApiEmbedder(memoryDraft.embedder) && !hasApiKeyForEmbedder(memoryDraft.embedder)) {
      ...
      return `Add ${article} ${keyLabel} installation API key, then Save & check.`;
    }
    return 'Choose one installation-wide embedder, then Save & check.';
  }
  ...
  function memoryNotice(): MemoryNoticeState | null {
    ...
    if (memoryChangeNeedsRebuild()) {
      return {
        ...
        detail: `${settings.memory.indexed_vectors} installation vectors use ${localEmbedderLabel(settings.memory.embedder)}. Changing embedder or embedding model needs a rebuild before it is safe.`,
      };
    }
    if (memoryDraftDirty()) {
        ...
        return {
          ...
          detail:
            hasKey
              ...
              : `Paste ${article} ${keyLabel} installation API key in Access, then Save & check.`,
      ...
      return {
        ...
        detail: `Save memory or Save & check to apply ${label} across this installation. No cloud API key is needed for this embedder.`,
      };
    }
    ...
    return {
      ...
      detail: needsApiKey
        ? `Paste ${article} ${keyLabel} installation API key in Access and click Connect key, or choose Local CPU.`
