...
class TestRejectsTooShort:
    def test_rejects_very_short_default_salience(self):
        result = check_quality("hi", salience=5.0, skip_duplicate_check=True)
        assert not result.passed
        assert "too short" in result.reason

    def test_rejects_empty(self):
        result = check_quality("", salience=5.0, skip_duplicate_check=True)
        assert not result.passed

    def test_allows_short_with_high_salience(self):
        result = check_quality("critical note", salience=9.0, skip_duplicate_check=True)
        assert result.passed

    def test_rejects_short_with_low_salience(self):
        result = check_quality("some words", salience=3.0, skip_duplicate_check=True)
        assert not result.passed
        assert "too short" in result.reason


class TestRejectsTestArtifacts:
    def test_rejects_cap_test(self):
        result = check_quality("cap test", salience=10.0, skip_duplicate_check=True)
        assert not result.passed
        assert "test/debug artifact" in result.reason

    def test_rejects_test_encode(self):
        result = check_quality("test encode", salience=5.0, skip_duplicate_check=True)
        assert not result.passed

    def test_rejects_hello_world(self):
        result = check_quality("hello world", salience=5.0, skip_duplicate_check=True)
        assert not result.passed


class TestRejectsNearDuplicate:
    @patch("brain.systems.quality.gate._check_near_duplicate")
    def test_rejects_when_duplicate_found(self, mock_dup):
    ...
    @patch("brain.systems.quality.gate._check_near_duplicate")
    def test_passes_when_no_duplicate(self, mock_dup):
...
class TestAcceptsValidMemory:
    ...
    def test_accepts_medium_content(self):
        result = check_quality(
...
class TestAutoAdjustsSalience:
    ...
    def test_quality_gate_returns_adjusted(self):
        result = check_quality(
