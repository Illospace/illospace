...
def test_create_github_issue_guidance_is_primitive_operation_and_failure_contract_only():
    ...
    assert "assignee_rationale is required" in properties["assignees"]["description"]
    assert "Unassigned is the correct fallback" in properties["assignees"]["description"]
    assert "assignee_rationale" in properties
...
async def test_create_github_issue_happy_path_opens_real_issue():
    ...
    with bind_agent_context({"user_id": "u", "org_id": "o"}), p1, p2, p3, patch(
        ...
        result = await _handle_create_github_issue(
            ...
            labels=["bug"],
            assignees=["reda"],
            assignee_rationale="Backend ownership playbook assigns this area to Reda.",
        )
    ...
    assert payload["assignee_gate"] == "justified"
    assert "assignees_dropped" not in payload
...
@pytest.mark.asyncio
@pytest.mark.parametrize("assignee_rationale", [None, " \t "])
async def test_create_github_issue_drops_assignee_without_rationale(assignee_rationale):
    created = {
        "repo": "uwear-ai/uwear-backend",
        "issue": {"type": "issue", "number": 322},
    }
    p1, p2, p3 = _vault_patches(bound_env={"GITHUB_TOKEN": "write-token"}, secrets=[])
    with bind_agent_context({"user_id": "u", "org_id": "o"}), p1, p2, p3, patch(
        f"{_H}.async_create_repo_issue",
    ...
    payload = json.loads(result)
    assert payload["issue"]["number"] == 322
    assert payload["assignee_gate"] == "unjustified"
    assert payload["assignees_dropped"] == ["redawear"]
    assert create.await_args.kwargs["assignees"] == []


@pytest.mark.asyncio
async def test_create_github_issue_without_assignee_has_no_gate_metadata():
    created = {
        "repo": "uwear-ai/uwear-backend",
        "issue": {"type": "issue", "number": 323},
    }
    p1, p2, p3 = _vault_patches(bound_env={"GITHUB_TOKEN": "write-token"}, secrets=[])
    with bind_agent_context({"user_id": "u", "org_id": "o"}), p1, p2, p3, patch(
        f"{_H}.async_create_repo_issue",
    ...
    payload = json.loads(result)
    assert payload["issue"]["number"] == 323
    assert "assignee_gate" not in payload
    assert "assignees_dropped" not in payload
    assert create.await_args.kwargs["assignees"] == []
