...
def _make_uow(rows):
    ...
    async def _execute(*_args, **_kwargs):
        return result
    uow.session.execute.side_effect = _execute
    return uow


class TestDivergenceDetection:
    @patch("brain.jobs.pipelines.divergence.UnitOfWork")
    async def test_detect_overlapping_topics(self, MockUoW):
        ...
        results = await detect_divergence(date(2026, 3, 15), org_id="org-1")
        # auth overlap: shared={"auth"}, but need >= 2 shared tags, so no overlap detected
        assert isinstance(results, list)

    @patch("brain.jobs.pipelines.divergence.UnitOfWork")
    async def test_detect_strong_overlap(self, MockUoW):
        MockUoW.return_value = _make_uow([
    ...
    @patch("brain.jobs.pipelines.divergence.UnitOfWork")
    async def test_single_user_returns_empty(self, MockUoW):
        MockUoW.return_value = _make_uow([
            {"user_id": "u1", "user_name": "Alice", "topic_tags": ["auth"], "content_sample": "Work"},
        ])

        from brain.jobs.pipelines.divergence import detect_divergence
        assert await detect_divergence(date(2026, 3, 15), org_id="org-1") == []
