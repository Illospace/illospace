...
class TestConsolidationPerOrg:
    def test_phase_consolidation_accepts_org_id(self, mock_nightly_uow):
        with patch("brain.jobs.pipelines.consolidate.UnitOfWork", return_value=mock_nightly_uow):
            ...
            phase_consolidation(date(2026, 3, 15), org_id=ORG_ID)
    ...
    def test_phase_consolidation_no_org_id(self, mock_nightly_uow):
        """Legacy mode — no org_id — still works."""
        with patch("brain.jobs.pipelines.consolidate.UnitOfWork", return_value=mock_nightly_uow):
            from brain.jobs.pipelines.consolidate import phase_consolidation
            phase_consolidation(date(2026, 3, 15))


class TestDreamPerOrg:
    def test_gather_memories_filters_by_org(self, mock_nightly_uow):
        with patch("brain.jobs.pipelines.nightly_dream.UnitOfWork", return_value=mock_nightly_uow):
            ...
            gather_today_memories(date(2026, 3, 15), org_id=ORG_ID)
    ...
    def test_gather_today_memories_no_org(self, mock_nightly_uow):
        """Without org_id, query should NOT filter by org_id."""
        with patch("brain.jobs.pipelines.nightly_dream.UnitOfWork", return_value=mock_nightly_uow):
            from brain.jobs.pipelines.nightly_dream import gather_today_memories
            gather_today_memories(date(2026, 3, 15))
        query_str = str(mock_nightly_uow.session.execute.call_args[0][0])
        assert "AND org_id" not in query_str

    def test_gather_random_old_memories_filters_by_org(self, mock_nightly_uow):
        with patch("brain.jobs.pipelines.nightly_dream.UnitOfWork", return_value=mock_nightly_uow):
            from brain.jobs.pipelines.nightly_dream import gather_random_old_memories
            gather_random_old_memories(date(2026, 3, 15), org_id=ORG_ID)
        query_str = str(mock_nightly_uow.session.execute.call_args[0][0])
        assert "org_id" in query_str

    def test_gather_random_old_memories_no_org(self, mock_nightly_uow):
        with patch("brain.jobs.pipelines.nightly_dream.UnitOfWork", return_value=mock_nightly_uow):
            from brain.jobs.pipelines.nightly_dream import gather_random_old_memories
            gather_random_old_memories(date(2026, 3, 15))
        query_str = str(mock_nightly_uow.session.execute.call_args[0][0])
        assert "AND org_id" not in query_str


class TestReflectPerOrg:
    def test_gather_context_accepts_org_id(self, mock_nightly_uow):
        with patch("brain.jobs.pipelines.nightly_reflect.UnitOfWork", return_value=mock_nightly_uow), \
            ...
            try:
                result = gather_context(date(2026, 3, 15), org_id=ORG_ID)
            except Exception:
...
class TestGetAllOrgs:
    def test_get_all_orgs_returns_list(self, mock_uow):
        ...
        mock_org2.name = "Beta"
        mock_org2.slug = "beta"
        mock_uow.session.scalars.return_value.all.return_value = [mock_org1, mock_org2]
    ...
    def test_get_all_orgs_empty(self, mock_uow):
