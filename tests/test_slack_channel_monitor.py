...
def test_contact_form_lead_is_classified_by_shape_with_reordered_fields_and_no_phone():
...
def test_contact_form_lead_is_classified_from_slack_blocks():
    from brain.systems.slack.ingress import normalize_slack_socket_event
        ...
        _socket_mode_channel_message(
            ...
            blocks=[
                ...
                },
                {
                    "type": "section",
                    "fields": [
                        {"type": "mrkdwn", "text": "*Company Website:*\nwww.madamedusk.com"},
...
def test_rollbar_alert_in_same_monitored_channel_remains_byte_identical():
    ...
    assert envelope == {
        ...
        "desired_outcome": (
            "Illo should handle the request and reply in Slack when a visible "
            "response fits."
        ),
        "idempotency_key": "slack:T789:C_ALERTS:1716900000.000200",
    }
...
def test_contact_form_lead_builds_threaded_dossier_with_named_owner_and_next_action():
    from brain.systems.slack.triggers import build_slack_work_intake_payload

    lead_payload = _channel_monitor_payload()
    lead_payload.update(
        {
    ...
    assert metadata["contact_form_lead_dossier"].startswith("*Contact-form lead*")
...
def test_contact_form_lead_owner_defaults_to_reda_without_identity_mapping():
    from brain.systems.slack.contact_form_leads import contact_form_lead_owner

    owner = contact_form_lead_owner(_FakeConnection(metadata={}))
...
def test_aline_athaydes_replay_lists_capabilities_and_qwen_verification_separately():
    from brain.systems.slack.contact_form_leads import contact_form_lead_dossier
...
class _FakeConnection:
    def __init__(self, metadata=None, org_id="org1", owner_user_id="user-reda"):
        self.id = "conn1"
        self.org_id = org_id
...
@pytest.mark.asyncio
async def test_contact_form_lead_gets_eyes_and_routes_to_reda_in_one_sweep(monkeypatch):
    connector_module, reactions, submitted = _patch_connector(monkeypatch)
...
@pytest.mark.asyncio
async def test_process_socket_payload_records_named_owner_thread_reply(monkeypatch):
    ...
    connector_module, _reactions, _submitted = _patch_connector(monkeypatch)
    connection = _FakeConnection(
