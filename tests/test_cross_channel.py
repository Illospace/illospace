...
@pytest.fixture
def seed_memories(db_session, unit_of_work_for_session):
    """Insert test memories with source_session inside a rolled-back transaction."""
    now = datetime.now(timezone.utc)
    test_ids = []

    def ensure_principal(org_id: str, user_id: str, slug: str, email: str) -> None:
    ...
    ensure_principal(ORG_ID, USER_ID, "cross-channel", "cross-channel@example.com")
    ensure_principal(OTHER_ORG_ID, OTHER_USER_ID, "cross-channel-other", "cross-channel-other@example.com")

    test_ids.append(insert_memory(
...
class TestGetCrossChannelContext:
    ...
    def test_returns_memories_from_other_sessions(self, seed_memories):
        results = get_cross_channel_context(
            "telegram:direct:999",
            hours=24,
            user_id=USER_ID,
            org_id=ORG_ID,
    ...
    def test_excludes_current_session(self, seed_memories):
        results = get_cross_channel_context(
            "telegram:direct:999",
            hours=24,
            user_id=USER_ID,
    ...
    def test_respects_hours_parameter(self, seed_memories):
        results = get_cross_channel_context(
            "telegram:direct:999",
            hours=3,
            user_id=USER_ID,
    ...
    def test_excludes_old_memories(self, seed_memories):
        results = get_cross_channel_context(
            "telegram:direct:999",
            hours=24,
            user_id=USER_ID,
    ...
    def test_applies_memory_visibility_scope(self, seed_memories):
        results = get_cross_channel_context(
            "telegram:direct:999",
            hours=24,
            user_id=USER_ID,
