...
- Tool schema (`definitions/cortex_thread.py`): `target_tool` enum
  ...
  per-member map (env `ILLO_MEMBER_AGENT_TARGETS`, **keyed by user-id UUID**
  like every owner env in this repo — `<uuid>=claude,<uuid>=codex` — labels
  resolve via the `_fill_owner_labels` pattern in
  `change_notifications_cycle.py`; never short names).
...
- Route tests: redirect for codex; HTML page (200, contains prompt, no
  ...
  semantics per target: codex → on redirect (as today); claude → on the
  copy button (a POST), NOT on page render — a curiosity view must not
  count as a launch (slice 07 depends on this). Note: the launch link
  requires an authenticated Illo session (`get_current_user`), including on
  phones — the page is not anonymous.
