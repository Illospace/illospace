...
class CycleRepository(BaseRepository[Cycle]):
    ...
    def list_for_user(self, user_id: str) -> Sequence[Cycle]:
        stmt = (
            select(Cycle)
    ...
    def get_for_user(self, cycle_id: int, user_id: str) -> Cycle | None:
...
class CycleRunRepository(BaseRepository[CycleRun]):
    ...
    def list_for_cycle(self, cycle_id: int, *, limit: int = 25) -> Sequence[CycleRun]:
        stmt = (
            select(CycleRun)
