...
class ChatConversationRepository(BaseRepository[ChatConversation]):
    ...
    async def _a_approved_users_by_id(
        self,
        org_id: str,
        user_ids: Sequence[str],
    ) -> dict[str, User]:
    ...
    async def a_get_by_stable_key(self, org_id: str, stable_key: str) -> ChatConversation | None:
        stmt = select(ChatConversation).where(
            ChatConversation.org_id == org_id,
            ChatConversation.stable_key == stable_key,
        )
    ...
    async def a_get_for_user(self, conversation_id: str, user_id: str) -> ChatConversation | None:
        stmt = (
            select(ChatConversation)
            .join(
                ChatConversationMember,
    ...
    async def a_list_for_user(self, org_id: str, user_id: str) -> Sequence[ChatConversation]:
        stmt = (
            select(ChatConversation)
            .join(
                ChatConversationMember,
    ...
    async def a_ensure_org_room(
        ...
        *,
        ...
        conversation = await self.a_get_by_stable_key(org_id, "org-room")
        if conversation is None:
            try:
        ...
        if not conversation.title:
            conversation.title = title
        await self.a_sync_org_room_members(conversation.id, org_id)
        return conversation
    ...
    async def a_sync_org_room_members(self, conversation_id: str, org_id: str) -> None:
    ...
    async def a_get_or_create_dm(
        ...
        approved_users = await self._a_approved_users_by_id(
            org_id,
            [initiating_user_id, other_user_id],
        )
        if set(approved_users) != {initiating_user_id, other_user_id}:
        ...
        stable_key = build_dm_stable_key(initiating_user_id, other_user_id)
        conversation = await self.a_get_by_stable_key(org_id, stable_key)
        if conversation is None:
            try:
                async with self._session.begin_nested():
                    conversation = ChatConversation(
    ...
    async def a_list_member_users(self, conversation_id: str) -> Sequence[User]:
        stmt = (
            select(User)
            .join(
                ChatConversationMember,
    ...
    async def a_list_member_ids(self, conversation_id: str) -> list[str]:
        stmt = select(ChatConversationMember.user_id).where(
            ChatConversationMember.conversation_id == conversation_id
        )
        return [str(user_id) for user_id in (await self._session.scalars(stmt)).all()]
...
class ChatMessageRepository(BaseRepository[ChatMessage]):
    ...
    async def _a_conversation_for_update(self, conversation_id: str) -> ChatConversation:
        stmt = (
            select(ChatConversation)
            .where(ChatConversation.id == conversation_id)
            .with_for_update()
    ...
    async def _a_message_for_update(self, message_id: int) -> ChatMessage | None:
        stmt = (
            select(ChatMessage)
            .where(ChatMessage.id == message_id)
            .with_for_update()
    ...
    async def _a_validate_sender_membership(
        self,
        *,
        conversation_id: str,
        sender_user_id: str | None,
    ...
    async def _a_resolve_thread_root(
        self,
        *,
        conversation: ChatConversation,
        thread_root_message_id: int | None,
        ...
        root_message = await self._a_message_for_update(thread_root_message_id)
        if (
            root_message is None
            or root_message.deleted_at is not None
            or root_message.conversation_id != conversation.id
    ...
    async def _a_normalize_reply_target_id(
        self,
        *,
        conversation_id: str,
        root_message: ChatMessage | None,
        ...
        reply_target = await self._a_message_for_update(reply_to_message_id)
        if (
            reply_target is None
            or reply_target.deleted_at is not None
            or reply_target.conversation_id != conversation_id
    ...
    async def a_list_conversation_messages(
        self,
        conversation: ChatConversation,
        *,
        before_seq: int | None = None,
        ...
        if before_seq is not None:
            stmt = stmt.where(ChatMessage.conversation_seq < before_seq)
        stmt = stmt.order_by(ChatMessage.conversation_seq.desc()).limit(limit)
        return list(reversed((await self._session.scalars(stmt)).all()))
    ...
    async def a_list_thread_replies(
        self,
        root_message_id: int,
        *,
        before_seq: int | None = None,
    ...
    async def a_list_thread_preview_sender_ids(
        self,
        root_message_ids: Sequence[int],
        *,
        limit_per_thread: int = 2,
        ...
        previews: dict[int, list[str]] = {}
        seen_by_root: dict[int, set[str]] = {}
        for root_message_id, sender_user_id in (await self._session.execute(stmt)).all():
            if root_message_id is None or sender_user_id is None:
                continue
    ...
    async def a_get_last_message(self, conversation_id: str) -> ChatMessage | None:
        stmt = (
            select(ChatMessage)
            .where(
                ChatMessage.conversation_id == conversation_id,
    ...
    async def a_create_message(
        ...
        *,
        ...
        now = datetime.now(timezone.utc)
        conversation = await self._a_conversation_for_update(conversation_id)
        sender_user_id = str(sender_user_id) if sender_user_id is not None else None
        await self._a_validate_sender_membership(
            conversation_id=conversation_id,
            sender_user_id=sender_user_id,
        )
        root_message = await self._a_resolve_thread_root(
            conversation=conversation,
            thread_root_message_id=thread_root_message_id,
        )
        normalized_reply_to_message_id = await self._a_normalize_reply_target_id(
            conversation_id=conversation_id,
            root_message=root_message,
            reply_to_message_id=reply_to_message_id,
        )
        ...
        if sender_user_id is not None:
            await ChatConversationReadRepository(self._session).a_upsert_cursor(
                conversation_id=conversation_id,
                user_id=sender_user_id,
                last_read_conversation_seq=message.conversation_seq,
                last_read_message_id=message.id,
...
class ChatMessageMentionRepository(BaseRepository[ChatMessageMention]):
    ...
    async def a_mark_seen_through_conversation_seq(
        self,
        *,
        user_id: str,
        conversation_id: str,
...
class ChatNotificationRepository(BaseRepository[ChatNotification]):
    ...
    async def a_list_for_user(self, user_id: str, *, limit: int = 50) -> Sequence[ChatNotification]:
        stmt = (
            select(ChatNotification)
            .where(ChatNotification.user_id == user_id)
            .order_by(ChatNotification.created_at.desc())
    ...
    async def a_mark_read(self, notification_id: int, user_id: str) -> ChatNotification | None:
        ...
        notification = (await self._session.scalars(stmt)).first()
        if notification is None:
            return None
        if notification.read_at is None:
            notification.read_at = datetime.now(timezone.utc)
    ...
    async def a_mark_read_through_conversation_seq(
        self,
        *,
        user_id: str,
        conversation_id: str,
...
class ChatConversationReadRepository(BaseRepository[ChatConversationRead]):
    ...
    async def a_unread_counts_for_users(
        self,
        *,
        org_id: str,
        user_ids: Sequence[str],
    ...
    async def _a_normalized_cursor(
        self,
        *,
        conversation_id: str,
        last_read_conversation_seq: int,
    ...
    async def a_get_for_user(self, conversation_id: str, user_id: str) -> ChatConversationRead | None:
        stmt = select(ChatConversationRead).where(
            ChatConversationRead.conversation_id == conversation_id,
            ChatConversationRead.user_id == user_id,
        )
    ...
    async def a_upsert_cursor(
        ...
        *,
        ...
        now = datetime.now(timezone.utc)
        normalized_seq, normalized_message_id = await self._a_normalized_cursor(
            conversation_id=conversation_id,
            last_read_conversation_seq=last_read_conversation_seq,
            last_read_message_id=last_read_message_id,
        )
