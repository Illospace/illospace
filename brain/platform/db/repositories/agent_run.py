...
class AgentRunRepository(BaseRepository[AgentRunRow]):
    ...
    def list_by_thread(self, thread_id: str, *, limit: int = 50) -> Sequence[AgentRunRow]:
        stmt = (
            select(AgentRunRow)
    ...
    def list_claimable(self, *, limit: int = 25) -> Sequence[AgentRunRow]:
        stmt = (
            select(AgentRunRow)
...
class AgentRunEventRepository(BaseRepository[AgentRunEventRow]):
    ...
    def list_after(self, *, cursor: int = 0, limit: int = 100) -> Sequence[AgentRunEventRow]:
        stmt = (
    ...
    def list_for_run(self, run_id: int, *, limit: int = 200) -> Sequence[AgentRunEventRow]:
        stmt = (
            select(AgentRunEventRow)
...
class AgentRunArtifactRepository(BaseRepository[AgentRunArtifactRow]):
    ...
    def list_for_run(self, run_id: int) -> Sequence[AgentRunArtifactRow]:
        stmt = (
