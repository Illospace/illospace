...
# Every AI-written body opens with a blockquote banner, but the tail varies by the
# routine that wrote it ("during triage.", "during an autonomous run.", "during a
# routine run.", "during triage (pilot verification sweep, 2026-08-19).").
# Measured across 24 banners on live issues: matching the full triage sentence
# caught 17 and missed 7. Only this opening is invariant, so only it is matched.
GITHUB_AI_PROVENANCE_BANNER_PREFIX = "This was generated by AI"
...
def _comment_authorship(body: str) -> str:
    ...
    banner = first_line[1:].lstrip().lstrip("*").lstrip()
    if banner.casefold().startswith(GITHUB_AI_PROVENANCE_BANNER_PREFIX.casefold()):
        return "automation"

    # Absence of the banner is not evidence that a human wrote the comment. Shared
    # credentials make authorship unknowable without affirmative provenance.
    return "unknown"
