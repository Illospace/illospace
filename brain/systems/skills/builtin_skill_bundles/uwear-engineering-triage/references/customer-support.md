...
2. **Form a hypothesis.** State a concrete, evidence-backed reason for the result
   ...
   it).
3. **Open the durable work item.** File a **real GitHub issue in the owning repo**
   (`uwear-ai/uwear-backend` for generation/API behavior) with the payload
   evidence + hypothesis, pointing the owner at the payload to confirm or deny.
   Carry the customer's quote, credit-loss or other concrete impact, and the
   Slack `origin_ref` into the body. If the human explicitly requested an
   assignee, use that person's verified GitHub identity after the hypothesis is
   formed. Prefix the AI body with
   ...
   handoff, then name the GitHub-issue blocker in the reply) — never claim a
   filed issue.
4. **Mirror the issue.** After GitHub succeeds, create a linked tracker record in
   existing Domain `1` with the issue number/URL and stable external id. Never
   create a Domain as part of filing. If a specialized tracker seems useful,
   propose it separately and use Domain `1` for the immediate mirror.
5. **Reply in-thread** on the alert with the issue + tracker link and a one-line
   hypothesis.
