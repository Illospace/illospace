...
async def _handle_session_write(section: str, value: str, key: str | None = None) -> str:
    ...
    async with UnitOfWork() as uow:
        entry = await uow.scratchpad.write(
...
async def _handle_session_read(section: str | None = None, key: str | None = None) -> str:
    ...
    async with UnitOfWork() as uow:
        entries = await uow.scratchpad.read(run_id=run_id, section=section, key=key)
        return json.dumps({"run_id": run_id, "count": len(entries), "entries": entries})


async def _handle_read_thread_messages(
    mode: str = "recent",
    ...
    from brain.systems.sessions import async_read_thread_messages

    payload = await async_read_thread_messages(
        str(session_id),
        mode=mode,
...
async def _handle_session_append(section: str, value: str) -> str:
    """Shorthand for session_write without a key."""
    return await _handle_session_write(section=section, value=value, key=None)


async def _handle_session_list(section: str | None = None) -> str:
    ...
    async with UnitOfWork() as uow:
        if section:
            entries = await uow.scratchpad.read(run_id=run_id, section=section)
            return json.dumps({"run_id": run_id, "section": section, "count": len(entries), "entries": entries})
        sections = await uow.scratchpad.list_sections(run_id=run_id)
...
async def _handle_session_promote() -> str:
    ...
    async with UnitOfWork() as uow:
        return json.dumps(await uow.scratchpad.promote(run_id=run_id))


async def _handle_session_close() -> str:
    ...
    async with UnitOfWork() as uow:
        count = await uow.scratchpad.close(run_id=run_id)
