...
def _expand_lazy_loaded_results(
    ...
    *,
    ...
    if retrieval_decision_id is None:
        return []
    return controller.load_lazy_candidates(
...
def add_memory(
    ...
    qr = check_quality(content, salience=salience, memory_type=memory_type)
    if not qr.passed:
    ...
    with UnitOfWork() as uow:
        ...
        return uow.memories.insert_memory(
            content=content,
...
def query_memories(
    ...
    with UnitOfWork() as uow:
        ...
        retrieval = uow.memories.query_ranked(
    ...
    if _lazy_load_enabled(expand_lazy_load) and selection.lazy_load_eligible:
        lazy_loaded_results = _expand_lazy_loaded_results(
            AttentionController(),
...
def cmd_context(args):
    result = get_context(args.id, depth=args.depth)
    print(json.dumps(result, indent=2, default=str))


def cmd_connect(args):
    eid = connect_memories(args.source, args.target, args.rel, args.weight)
...
def cmd_import_md(args):
    ...
    for section in sections[1:]:
        ...
        result = add_memory(content=dense, memory_type=mtype, salience=sal,
