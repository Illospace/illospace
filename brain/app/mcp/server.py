...
async def tool_manage_runtime_preferences(
    action: str = "get",
    ...
    """Inspect or persist one explicitly supported durable preference."""
    from brain.platform.db.models.org import User
    from brain.systems.runtime_settings.display import (
    ...
    async with UnitOfWork() as uow:
        if normalized_action == "set":
            ...
            if user is None or (org_id and str(getattr(user, "org_id", "")) != str(org_id)):
                return {
                    "action": "set",
                    "status": "denied",
                    "saved": False,
                    "setting": DISPLAY_TIMEZONE_SETTING_KEY,
                    "detail": "The display preference was not saved: authenticated workspace authority is required.",
                }
            if not can_manage_runtime_settings(user):
                return {
                    ...
                    "saved": False,
                    "setting": DISPLAY_TIMEZONE_SETTING_KEY,
                    "detail": "The display preference was not saved: owner or admin authority is required.",
                }
            display = await async_update_runtime_display(
                uow.session,
                user,
                RuntimeDisplayUpdate(display_timezone=str(value or "")),
            )
            return {
                "action": "set",
                "status": "saved",
        ...
        if normalized_setting:
            ...
            display = await async_get_runtime_display(uow.session)
            return {
                "action": "get",
                "status": "ok",
                "setting": DISPLAY_TIMEZONE_SETTING_KEY,
...
TOOLS = {
    ...
    "manage_runtime_preferences": {
        "function": tool_manage_runtime_preferences,
        "description": (
            "Inspect or persist a supported durable workspace preference. The known writable "
            "setting is display_timezone. Owner/admin only for writes."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "setting": {
                    "type": "string",
                    "description": "Concrete preference key, currently display_timezone.",
                },
                "value": {
                    "type": "string",
                    "description": "IANA timezone or ET/Eastern alias for display_timezone.",
                },
            },
        },
    },
