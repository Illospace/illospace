...
@router.get("/keys")
def list_api_keys(request: Request, user: dict[str, Any] = Depends(get_current_user)):
    user_id = user.get("id")
    if not user_id:
...
def auto_import_keys(request: Request, user: dict[str, Any] = Depends(get_current_user)):
    ...
    imported = []
    with UnitOfWork() as uow:
        # Check if user already has a personal key
        stmt = (
        ...
        if not uow.session.scalars(stmt).first():
            ...
            # Use text() for ON CONFLICT with complex DO UPDATE
            uow.session.execute(
                text("""
        ...
        # Check if org already has a key (owner only)
...
async def add_api_key(request: Request, user: dict[str, Any] = Depends(get_current_user)):
    ...
    key_id = set_api_key(user_id, api_key, provider=provider, label=label)
    return {
        "id": key_id,
        "status": "stored",
...
async def set_default_key(request: Request, user: dict[str, Any] = Depends(get_current_user)):
    ...
    with UnitOfWork() as uow:
        if api_key_id is not None:
            # Check the key is accessible (owned or shared)
            stmt = (
                select(UserApiKey.id)
                .where(
...
async def set_org_main_key(request: Request, user: dict[str, Any] = Depends(get_current_user)):
    ...
    store_org_api_key(org_id, provider, encrypted, uow_factory=UnitOfWork)
    return {
...
async def share_key(key_id: int, request: Request, user: dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        share_id = share_api_key(key_id, target_user_id, user_id)
...
def deactivate_key(key_id: int, user: dict[str, Any] = Depends(get_current_user)):
    user_id = user.get("id")
    if not user_id:
        raise HTTPException(status_code=401, detail="Not authenticated")
    with UnitOfWork() as uow:
